...
def image_evaluation(gen_conf, test_conf):
    ...
    im_recon = read_result_volume(gen_conf, test_conf) # shape : (subject, modality, mri_shape)

    # compare image and get stats
    compare_images_and_get_stats(gen_conf, test_conf, im_gt, im_recon)
    if ishomo == True:
        compare_images_and_get_stats(gen_conf, test_conf, im_gt, im_interp, save_filename="stat_brain_interp.csv")
...
def compare_images_and_get_stats(gen_conf, test_conf, im_gt, im_recon, save_filename = 'stats_brain.csv'):
