...
def __generate_unet_model(
    ...
    conv1 = get_conv_core(dimension, input, int(64/downsize_factor)) ## 32
    pool1 = get_max_pooling_layer(dimension, conv1)

    conv2 = get_conv_core(dimension, pool1, int(128/downsize_factor)) ## 64
    pool2 = get_max_pooling_layer(dimension, conv2)

    conv3 = get_conv_core(dimension, pool2, int(256/downsize_factor)) ## 128
    pool3 = get_max_pooling_layer(dimension, conv3)

    conv4 = get_conv_core(dimension, pool3, int(512/downsize_factor)) ## 256

    up5 = get_deconv_layer(dimension, conv4, int(256/downsize_factor)) ## 128
    up5 = concatenate([up5, conv3], axis=1)

    conv5 = get_conv_core(dimension, up5, int(256/downsize_factor)) ## 128

    up6 = get_deconv_layer(dimension, conv5, int(128/downsize_factor)) ## 64
    up6 = concatenate([up6, conv2], axis=1)

    conv6 = get_conv_core(dimension, up6, int(128/downsize_factor)) ## 64

    up7 = get_deconv_layer(dimension, conv6, int(64/downsize_factor)) ## 32
    up7 = concatenate([up7, conv1], axis=1)

    ## Comment the FCNN layer.
    # conv7 = get_conv_core(dimension, up7, int(64/downsize_factor)) ## 32

    pred = get_conv_fc(dimension, up7, num_modalities)
