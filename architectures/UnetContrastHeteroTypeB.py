...
def generate_hetero_unet_model(gen_conf, train_conf) :
    ...
    model = __generate_hetero_unet_model22(
...
def __generate_hetero_unet_model22(
    ...
    if thickness_factor == 4:
        conv1 = get_conv_core(dimension, input, int(num_filters/downsize_factor)) ## c128
        ...
        # conv6 = get_conv_core(dimension, conv4, int(num_filters*16/np.prod(temp_sparse_scale)/downsize_factor)) ## c1024
        ...
        # conv7 = get_conv_core(dimension, conv3, int(num_filters*8/downsize_factor))  ## c1024
        ...
        temp_sparse_scale = sparse_scale
        conv9 = get_conv_core(dimension, up8, int(num_filters*2/downsize_factor)) ## c64
        up9 = get_deconv_layer(dimension, conv9, int(num_filters/downsize_factor)) ## c32
        up9 = concatenate([up9, conv1], axis=1) ## c32+32

        conv10 = get_conv_core(dimension, up9, int(num_filters/downsize_factor)) ## c32
        ...
        conv1 = get_conv_core(dimension, input, int(num_filters / downsize_factor))  ## c16
        ...
        # reshape1 = get_shuffling_operation2(dimension, conv5, shuffling_dim, temp_sparse_scale)  ## c256
        ...
        temp_sparse_scale = sparse_scale ## [1,1,8]
        conv9 = get_conv_core(dimension, up8, int(num_filters * 2 / downsize_factor))  ## c32
        up9 = get_deconv_layer(dimension, conv9,
                               int(num_filters / downsize_factor))  ## c16
        up9 = concatenate([up9, conv1], axis=1)  ## c16+16

        conv10 = get_conv_core(dimension, up9, int(num_filters / downsize_factor))  ## c16
        pred = get_conv_fc(dimension, conv10, num_modalities)  ## the FCNN layer.
...
def __generate_hetero_unet_model2(
    ...
    if thickness_factor == 4:
        ...
        temp_sparse_scale = sparse_scale ## [1,1,8]
        pool1 = get_max_pooling_layer(dimension, conv1, (2, 2, 1))
        conv1 = get_shuffling_operation(dimension, conv1, shuffling_dim, temp_sparse_scale)  ## c2
        conv2 = get_conv_core(dimension, pool1, int(num_filters * 2 / downsize_factor), num_kernel=num_kernels)  ## c32

        temp_sparse_scale = sparse_scale / [1, 1, 2] ## [1,1,4]
        pool2 = get_max_pooling_layer(dimension, conv2, (2, 2, 1))
        conv2 = get_shuffling_operation(dimension, conv2, shuffling_dim, temp_sparse_scale)  ## c8
        conv3 = get_conv_core(dimension, pool2, int(num_filters * 4 / downsize_factor), num_kernel=num_kernels)  ## c64

        temp_sparse_scale = sparse_scale / [1, 1, 4] ## [1,1,2]
        pool3 = get_max_pooling_layer(dimension, conv3, (2, 2, 1))
        conv3 = get_shuffling_operation(dimension, conv3, shuffling_dim, temp_sparse_scale)  ## c32
        conv4 = get_conv_core(dimension, pool3, int(num_filters * 8 / downsize_factor),
                              num_kernel=num_kernels)  ## c128

        temp_sparse_scale = sparse_scale / [1, 1, 8] ## [1,1,1]
        pool4 = get_max_pooling_layer(dimension, conv4)
        conv4 = get_shuffling_operation(dimension, conv4, shuffling_dim, temp_sparse_scale)  ## c128
        conv5 = get_conv_core(dimension, pool4, int(num_filters * 16 / downsize_factor))  ## c256
        ...
        conv6 = get_conv_core(dimension, reshape1,
                              int(num_filters * 8 / np.prod(temp_sparse_scale) / downsize_factor))  ## c256
        up6 = get_deconv_layer(dimension, conv6,
                               int(num_filters * 8 / np.prod(temp_sparse_scale) / downsize_factor))  ## c128
        up6 = concatenate([up6, conv4], axis=1)  ## c128+128

        temp_sparse_scale = sparse_scale / [1, 1, 4] ## [1,1,2]
        conv7 = get_conv_core(dimension, up6, int(num_filters * 8 / np.prod(temp_sparse_scale) / downsize_factor),
                              num_kernel=num_kernels)  ## c64
        up7 = get_deconv_layer(dimension, conv7,
                               int(num_filters * 4 / np.prod(temp_sparse_scale) / downsize_factor))  ## c32
        up7 = concatenate([up7, conv3], axis=1)  ## c32+32

        temp_sparse_scale = sparse_scale / [1, 1, 2] ## [1,1,4]
        conv8 = get_conv_core(dimension, up7, int(num_filters * 4 / np.prod(temp_sparse_scale) / downsize_factor),
                              num_kernel=num_kernels)  ## c16
        up8 = get_deconv_layer(dimension, conv8,
                               int(num_filters * 2 / np.prod(temp_sparse_scale) / downsize_factor))  ## c8
        up8 = concatenate([up8, conv2], axis=1)  ## c8+8

        temp_sparse_scale = sparse_scale ## [1,1,8]
        conv9 = get_conv_core(dimension, up8, int(num_filters * 2 / np.prod(temp_sparse_scale) / downsize_factor),
                              num_kernel=num_kernels)  ## c4
        up9 = get_deconv_layer(dimension, conv9,
                               int(num_filters / np.prod(temp_sparse_scale) / downsize_factor))  ## c2
...
# Shuffling operation:
def get_shuffling_operation2(dimension, input, shuffling_dim, sparse_scale) :
    """
